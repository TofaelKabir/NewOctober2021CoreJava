...
/*
 ...
 -- Integer.parseInt(v)
  ...
  2) How can you change a int value to String?
 -- String.valueOf(v)
 
 3) How can you change a String value to double?
 -- Double.parseDouble(v)
 
 4) Give an example for variable initialized where default type of access modifier is used.
 -- int age = 30;
 
 5) How can we convert a float or double to an int in Java? Give example.
...
public int multiplication(int e, float f) {
 ...
 6) Do we need to use 'this' keyword always in parameterized method?
-- If global and local variables are not same, we must have to use this keyword in parameterized method to make a relation between global and local variable. otherwise not.
